...
//
// Okay, I add the DOS assembly module for the historically inclined here (may
// rec.games.programmer suffer). If anyone feels the urge to port these to GNU
// GCC; either inline or as separate modules including Makefile support, be my
// guest.
//
// Module tmap.S include the inner loops for texture mapping, the interesting
// one being the floor/ceiling span rendering.
//
// There was another module in the source dump, fpfunc.S, that had both texture
// mapping and fixed point functions. It contained implementions both for i386
// and M68k. For brevity, I include only the i386 fixed point stuff below
...
_R_DrawColumn:
	...
	movl ebp,[_dc_yl]
	movl ebx,ebp
	movl edi,[ylookup + ebx * 4]
	movl ebx[_dc_x]
	addl edi,[columnofs + ebx * 4]
	...
	movl ebx,[_dc_iscale]
	shll sbx,9
	movl eax,OFFSET patch1+2     // convince tasm to modify code...
	movl [eax],ebx
	movl eax,OFFSET patch2+2     // convince tasm to modify code...
	movl [eax],ebx
	...
	.align 16
doubleloop:
	movel ecx,ebp                // begin calculating third pixel
patch1:
	addl ebp,12345678h           // advance frac pointer
	movb [edi],al                // write first panel
...
patch2:
	...
	addl edi,SCREENWIDHT * 2     // advance to third pixel destination
	movb bl,[esi + edx]          // get fourth pixel
	decl [loopcount]             // done with loop?
	movb al,[eax]                // color transform third pixel
	movb bl,[ebx]                // color transform fourth pixl
...
_R_DrawSpan:
	...
	movl ebx,[_ds_xstep]
	shll ebx,10
	andl ebx,0xFFFF0000h
	movl eax,[_ds_ystep]
	shrl eax,6
	andl eax,oxFFFFh
	orl ebx,eax

	movl eax,OFFSET hpatch1+2    // convince tasm to modify code...
	movl [eax],ebx
	movl eax,OFFSET hpatch2+2    // convince tasm to modify code...
	movl [eax],ebx

	// eax      aligned colormap
	// ebx      alinged colormap
	// ecx,edx  scratch
	// esi      virtual source
	// edi      moving destination pointer
	// ebp      frac
	...
	addl ebp,ebx                 // advance frac pointer
	andl edx,4096                // finish calcuation for fourth pixel
