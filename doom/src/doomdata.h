...
// only under the term of the DOOM Source Code License as
...
//
// DESCRIPTION:
//     All external data is definted here.
...
// Map level types.
// The following data structures define the persistent format used in the lumps
// of the WAD files.

// Lump order in a map WAD: Each map needs a couple of lumps to provide a
// complete scene geometry description.
enum {
...
// A LieDef, as used for editing, and as input to the BSP builder.
typedef struct {
  short v1;
  short b2;
  short flags;
...
// Set if already seen, thus draw in automap.
...
typedef struct {
  // Partition line from (x,y) to (x_+ dx, y + dy)
  short x;
  short y;
  short dx;
  short dy;

  // Bounding box for each child, clip against view frustrum.
  short bbox[2][4];
